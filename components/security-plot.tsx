...
export default function SecurityPlot({points,mode,volume,currency,theme,ariaLabel='График цены актива'}:SecurityPlotProps){
 ...
 const left=16,right=width<500?77:86,top=25,bottom=34,plotWidth=Math.max(1,width-left-right),plotHeight=Math.max(90,height-top-bottom-volumeHeight-(showVolume?18:0));
 ...
 const x=(time:number)=>data.length<2?left+plotWidth/2:left+(time-first.time)/Math.max(1,last.time-first.time)*plotWidth;
 ...
 const line=data.map((p,i)=>(i?'L':'M')+x(p.time).toFixed(2)+' '+y(p.close).toFixed(2)).join(' ');
 const area=last&&first?line+' L'+x(last.time).toFixed(2)+' '+(top+plotHeight)+' L'+x(first.time).toFixed(2)+' '+(top+plotHeight)+' Z':'';
 const index=focusTime!==null&&data.length?nearest(data,focusTime):null;
 const selected=index===null?null:data[index],selectedX=selected?x(selected.time):0;
 ...
 const spacing=data.slice(1).map((p,i)=>x(p.time)-x(data[i].time)).filter(gap=>gap>0);
 const minSpacing=spacing.length?Math.min(...spacing):plotWidth;
 const candleWidth=Math.max(.7,Math.min(10,plotWidth/Math.max(1,data.length)*.62,minSpacing*.7));
 ...
 function move(event:React.PointerEvent<SVGSVGElement>){
  ...
  const time=first.time+(Math.max(left,Math.min(left+plotWidth,local))-left)/plotWidth*(last.time-first.time);
  setFocusTime(data[nearest(data,time)].time);
 }
 ...
 return <div ref={container} className="security-plot" data-theme={theme} data-mode={mode}>
  {!data.length?<div className="security-plot-empty"><span>Нет точек для выбранного периода</span><small>График появится после получения данных источника.</small></div>:<>
   <svg viewBox={'0 0 '+width+' '+height} width="100%" height="100%" role="img" tabIndex={0} aria-label={ariaLabel+(selected?'. '+date(selected.time,true)+': '+format(selected.close)+' '+currency:'')} aria-describedby={'security-instructions-'+id} onPointerMove={move} onPointerDown={move} onPointerLeave={()=>setFocusTime(null)} onPointerCancel={()=>setFocusTime(null)} onBlur={()=>setFocusTime(null)} onKeyDown={key}>
    <defs>
    ...
    {dateIndices.map((i,j)=>data[i]&&<text key={i} className="security-axis-text" x={x(data[i].time)} y={height-9} textAnchor={j===0?'start':j===dateIndices.length-1?'end':'middle'}>{axisDate(data[i].time)}</text>)}
   </svg>
   ...
   {grouped&&<div className="security-aggregation-note">Свечи сгруппированы: {data.length} из {clean.length} баров</div>}
  </>}
  <span id={'security-instructions-'+id} className="security-plot-sr">Наведите указатель или коснитесь графика для значений. Стрелки влево и вправо выбирают точку; Home и End — начало и конец; Escape скрывает подсказку. Даты в UTC.</span>
 </div>;
}
