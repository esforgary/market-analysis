...
export default function SourcesPanel({health=[],checkedAt}:{health?:SourceHealth[];checkedAt?:string}){
 ...
 return <section className="panel sources-directory">
  ...
  <div className="filter-bar"><label className="field-search"><input aria-label="Поиск источника" placeholder="Редакция, организация или лента…" value={query} onChange={event=>setQuery(event.target.value)}/></label><select aria-label="Статус источника" value={mode} onChange={event=>setMode(event.target.value)}><option value="all">Все статусы</option><option value="ok">Доступны</option><option value="error">Недоступны</option></select></div>
